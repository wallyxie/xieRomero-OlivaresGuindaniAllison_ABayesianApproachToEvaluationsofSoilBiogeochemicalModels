...
fit_summary = function(stan_fit, stan_fit_ex, S_0, D_0, M_0, filename) {
    #Posteriors
    write.csv(stan_fit_ex, file = paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "posteriors", "S", S_0, "D", D_0, "M", M_0, ".csv", sep = "_"))
    #Summary
    ...
    write.csv(stan_fit_sum_write, file = paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "summary", "S", S_0, "D", D_0, "M", M_0, ".csv", sep = "_"))
    #Sampler parameters
    sampler_params <- get_sampler_params(stan_fit, inc_warmup = FALSE) #Access sampler values
    write.csv(sampler_params, file = paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "sampler_params", "S", S_0, "D", D_0, "M", M_0, ".csv", sep = "_"))
}
...
calc_ic = function(stan_fit, stan_fit_ex, S_0, D_0, M_0, filename) {
    ...
    sink(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "ic", "S", S_0, "D", D_0, "M", M_0, ".txt", sep = "_"))    
    cat("WAIC = ", waic, "\nLOO = ", loo, "\nLPML = ", LPML, "\np_waic = ", p_waic, "\np_loo = ", p_loo)
    sink()    
...
bayes_diagnostics = function(stan_fit, S_0, D_0, M_0, filename) {
    ...
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "CI_Ea", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = CI_plot1)
    CI_plot2 <- mcmc_areas(stan_fit.array, pars = c("a_DS", "a_SD", "a_M", "a_MS"), prob = 0.8, prob_outer = 0.95) + yaxis_text()
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "CI_a", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = CI_plot2)
    #Traceplot
    traceplot1 <- mcmc_trace(stan_fit.array, pars = c("Ea_S", "Ea_D", "Ea_M")) + yaxis_text()
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "trace_Ea", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = traceplot1)
    traceplot2 <- mcmc_trace(stan_fit.array, pars = c("a_DS", "a_SD", "a_M", "a_MS")) + yaxis_text()
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "trace_a", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = traceplot2)
    #Autocorrelation
    acf_plot1 <- mcmc_acf(stan_fit.array, pars = c("Ea_S", "Ea_D", "Ea_M")) + yaxis_text()
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "acf_Ea", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = acf_plot1)
    acf_plot2 <- mcmc_acf(stan_fit.array, pars = c("a_DS", "a_SD", "a_M", "a_MS")) + yaxis_text()
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "acf_a", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = acf_plot2)
    #Rhat plot
    stan_fit_coda <- stan2coda(stan_fit)
    pdf(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "Rhat", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"))
...
calc_r <- function(model_vector, data_vector, S_0, D_0, M_0, filename) {
    ...
    ssr = sum(ssr_vector)
    r_sq = 1 - (ssr / sst)
    cat(r_sq, file = paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "r_sq", "S", S_0, "D", D_0, "M", M_0, ".txt", sep = "_"))
    return(r_sq)
}
...
plot_fits <- function(stan_fit_ex, N_t, N_p, obs_times, pred_times, data_vector, S_0, D_0, M_0, filename) {
    ...
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "post", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = post_plot)    
    ...
    pre_plot <- pre_plot + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                   panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%H_%m_%d_%m_%Y"), filename, "pred", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = pre_plot)   
}
...
CON_fit1 <- stan("CON_adriana_pools4p1.stan", data = CON_dat1, iter = 40000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
CON_fit2 <- stan("CON_adriana_pools4p1.stan", data = CON_dat2, iter = 40000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
CON_fit3 <- stan("CON_adriana_pools4p1.stan", data = CON_dat3, iter = 40000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
CON_fit4 <- stan("CON_adriana_pools4p1.stan", data = CON_dat4, iter = 40000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
CON_fit5 <- stan("CON_adriana_pools4p1.stan", data = CON_dat5, iter = 40000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
CON_fit6 <- stan("CON_adriana_pools4p1.stan", data = CON_dat6, iter = 40000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
CON_fit7 <- stan("CON_adriana_pools4p1.stan", data = CON_dat7, iter = 40000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
