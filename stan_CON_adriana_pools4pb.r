...
bayes_diagnostics = function(stan_fit, S_0, D_0, M_0, filename) {
    ...
    #Pairs
    pairsplot1 <- pairs(stan_fit, pars = c("Ea_S", "Ea_D", "Ea_M")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "pairs_Ea", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = pairsplot1)
    pairsplot2 <- pairs(stan_fit, pars = c("a_DS", "a_SD", "a_M", "a_MS")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "pairs_a", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = pairsplot2)
    #Traceplot
    traceplot1 <- rstan::traceplot(stan_fit, pars = c("Ea_S", "Ea_D", "Ea_M")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "trace_Ea", "S", S_0, "D", D_0, "M", M_0, ".pdf", sep = "_"), plot = traceplot1)
    traceplot2 <- rstan::traceplot(stan_fit, pars = c("a_DS", "a_SD", "a_M", "a_MS")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
