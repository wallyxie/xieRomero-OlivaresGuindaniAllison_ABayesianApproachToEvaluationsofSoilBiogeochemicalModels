...
fit_summary = function(stan_fit, stan_fit_ex, S_0, D_0, M_0, E_0, filename) {
    #Posteriors
    write.csv(stan_fit_ex, file = paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "posteriors", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".csv", sep = "_"))
    #Summary
    ...
    write.csv(stan_fit_sum_write, file = paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "summary", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".csv", sep = "_"))
    #Sampler parameters
    sampler_params <- get_sampler_params(stan_fit, inc_warmup = FALSE) #Access sampler values
    write.csv(sampler_params, file = paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "sampler_params", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".csv", sep = "_"))
}
...
calc_ic = function(stan_fit, stan_fit_ex, S_0, D_0, M_0, E_0, filename) {
    ...
    sink(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "ic", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".txt", sep = "_"))    
    cat("WAIC = ", waic, "\nLOO = ", loo, "\nLPML = ", LPML, "\np_waic = ", p_waic, "\np_loo = ", p_loo)
    sink()    
...
bayes_diagnostics = function(stan_fit, S_0, D_0, M_0, E_0, filename) {
    ...
    CI_plot1 <- mcmc_areas(stan_fit.array, pars = c("Ea_V", "Ea_VU", "Ea_K", "Ea_KU"), prob = 0.8, prob_outer = 0.95) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "CI_Ea", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = CI_plot1)
    CI_plot2 <- mcmc_areas(stan_fit.array, pars = c("V_ref", "E_C_ref", "a_MS"), prob = 0.8, prob_outer = 0.95) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "CI_V_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = CI_plot2)
    CI_plot3 <- mcmc_areas(stan_fit.array, pars = c("V_U_ref"), prob = 0.8, prob_outer = 0.95) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "CI_V_U_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = CI_plot3)
    CI_plot4 <- mcmc_areas(stan_fit.array, pars = c("m_t"), prob = 0.8, prob_outer = 0.95) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "CI_m_t", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = CI_plot4)
    #Traceplot
    traceplot1 <- mcmc_trace(stan_fit.array, pars = c("Ea_V", "Ea_VU", "Ea_K", "Ea_KU")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "trace_Ea", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot1)
    traceplot2 <- mcmc_trace(stan_fit.array, pars = c("V_ref", "E_C_ref", "a_MS")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "trace_V_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot2)
    traceplot3 <- mcmc_trace(stan_fit.array, pars = c("V_U_ref")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "trace_V_U_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot3)
    traceplot4 <- mcmc_areas(stan_fit.array, pars = c("m_t")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "trace_m_t", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot4)
    #Autocorrelation
    acf_plot1 <- mcmc_acf(stan_fit.array, pars = c("Ea_V", "Ea_VU", "Ea_K", "Ea_KU")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "acf_Ea", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = acf_plot1)
    acf_plot2 <- mcmc_acf(stan_fit.array, pars = c("V_ref", "E_C_ref", "a_MS")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "acf_V_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = acf_plot2)
    acf_plot3 <- mcmc_acf(stan_fit.array, pars = c("V_U_ref")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "acf_V_U_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = acf_plot3)
    acf_plot4 <- mcmc_acf(stan_fit.array, pars = c("m_t")) + yaxis_text() + theme(axis.text = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "acf_m_t", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = acf_plot4)
    #Rhat plot
    stan_fit_coda <- stan2coda(stan_fit)
    pdf(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "Rhat", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"))
...
calc_r <- function(model_vector, data_vector, S_0, D_0, M_0, E_0, filename) {
    ...
    ssr = sum(ssr_vector)
    r_sq = 1 - (ssr / sst)
    cat(r_sq, file = paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "r_sq", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".txt", sep = "_"))
    return(r_sq)
}
...
plot_fits <- function(stan_fit_ex, N_t, N_p, obs_times, pred_times, data_vector, S_0, D_0, M_0, E_0, filename) {
    ...
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "post", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = post_plot)    
    ...
    ggsave(paste(format(Sys.time(),"%Y_%d_%m_%H_%M"), filename, "pred", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = pre_plot)   
}
...
AWB_fit1 <- stan("AWB_adriana_pools5.stan", data = AWB_dat1, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
AWB_fit2 <- stan("AWB_adriana_pools5.stan", data = AWB_dat2, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
AWB_fit3 <- stan("AWB_adriana_pools5.stan", data = AWB_dat3, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
AWB_fit4 <- stan("AWB_adriana_pools5.stan", data = AWB_dat4, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
AWB_fit5 <- stan("AWB_adriana_pools5.stan", data = AWB_dat5, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
AWB_fit6 <- stan("AWB_adriana_pools5.stan", data = AWB_dat6, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
...
AWB_fit7 <- stan("AWB_adriana_pools5.stan", data = AWB_dat7, iter = 60000, warmup = 20000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.99, stepsize = 0.01, max_treedepth = 15))
