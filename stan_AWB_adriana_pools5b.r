...
bayes_diagnostics = function(stan_fit, S_0, D_0, M_0, E_0, filename) {
    ...
    #Pairs
    pairsplot1 <- pairs(stan_fit, pars = c("Ea_V", "Ea_VU", "Ea_K", "Ea_KU")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "pairs_Ea", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = pairsplot1)
    pairsplot2 <- pairs(stan_fit, pars = c("V_ref", "E_C_ref", "a_MS")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "pairs_V_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = pairsplot2)
    pairsplot3 <- pairs(stan_fit, pars = c("V_U_ref")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "pairs_V_U_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = pairsplot3)
    pairsplot4 <- pairs(stan_fit, pars = c("m_t")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "pairs_m_t", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = pairsplot4)
    #Traceplot
    traceplot1 <- rstan::traceplot(stan_fit, pars = c("Ea_V", "Ea_VU", "Ea_K", "Ea_KU")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "trace_Ea", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot1)
    traceplot2 <- rstan::traceplot(stan_fit, pars = c("V_ref", "E_C_ref", "a_MS")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "trace_V_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot2)
    traceplot3 <- rstan::traceplot(stan_fit, pars = c("V_U_ref")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
    ggsave(paste(format(Sys.time(),"%Y_%m_%d_%H_%M"), filename, "trace_V_U_ref", "S", S_0, "D", D_0, "M", M_0, "E", E_0, ".pdf", sep = "_"), plot = traceplot3)
    traceplot4 <- rstan::traceplot(stan_fit, pars = c("m_t")) + theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))
...
AWB_fit2 <- stan("AWB_adriana_pools5.stan", data = AWB_dat2, iter = 50000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.9995, stepsize = 0.001, max_treedepth = 15))
...
AWB_fit7 <- stan("AWB_adriana_pools5.stan", data = AWB_dat7, iter = 50000, warmup = 10000, refresh = 1, chains = 4, seed = 1234, open_progress = "False", control = list(adapt_delta = 0.9995, stepsize = 0.001, max_treedepth = 15))
